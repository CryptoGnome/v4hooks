...
> Curated directory of Uniswap v4 hooks you can use. Not the official Uniswap hooklist address registry.

Site: ${site}
Catalog JSON: ${site}/hooks.json
Full dump: ${site}/llm-full.txt
Contribute: ${site.replace("https://", "https://github.com/")} (see CONTRIBUTING.md)
...
## Hooks
${hooks.map((h) => `- [${h.name}](${site}/hooks/${h.slug}): ${h.tagline}`).join("\n")}
`;
...
${hooks
  .map((h) =>
    [
      ...
      h.flags?.length ? `Flags: ${h.flags.join(", ")}` : null,
      h.website ? `Website: ${h.website}` : null,
      h.repo ? `Repo: ${h.repo}` : null,
      h.docs ? `Docs: ${h.docs}` : null,
      h.audit_url ? `Audit: ${h.audit_url}` : null,
      h.license ? `License: ${h.license}` : null,
